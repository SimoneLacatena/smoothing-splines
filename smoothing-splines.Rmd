...
## 1.6  Funzioni Polinomiali a Tratti e Splines 
...
<p> In generale si può rappresentare $f \in \mathbb{R}^d$ come una espansione di qualsiasi grande insieme di funzioni di base, e controllare la complessità applicando un regolarizzatore come quello visto precedentemente</p> 
...
$$
f(X_1,X_2) = \sum_{i=1}^{K_1}\sum_{j=1}^{K_2} \beta_{ij} h_{1i}(X_1)h_{2j}(X_2)
$$
...
<p> Vediamo di adattare la smoothing spline stimando i parametri di $\lambda$ e di $df$  in modo da ottenere un risultato ottimale.
