...
# Introduzione
Prima di discutere della struttura dell'algoritmo occorre introdurre dei concetti fondamentali, partendo dal definire il problema della regressione.
...
$$
<p> In questo modo è possibile ottenere curve non lineari. Non è tuttavia consigliabile utilizzare valori trppo grandi di $d$ poichè la curva diventerebbe troppo flessibile, nella maggior parte delle applicazioni è infatti suggeribile non andare oltre al grado 3 o 4.</p>
...
## Funzioni  a gradino
<p> L'uso di funzioni polinomiali definisce una struttura globale della funzioe non lineare della variabile predittrice $X$, per superare questo limite  è possibile utilizzare delle **funzioni a gradino**.</p>
<p>Tali funzioni dividono l'intervallo dei valori in $K$ bins distinti creando un insieme di valori di soglia ( detti anche **cutpoints**) $c_1 ,c_2, ..., c_k$ e per ogni bin si va ad adattare una funzione costante, Questo equivale a trasformare una variabile continua in una variabile categorica ordinale, avremo quindi: </p>
...
<p> I **gradi di libertà** per questo esempio sono 8, per gradi di libertà si intende il numero di parametri liberi, come il numero dei coefficenti in una polinomiale, nell'esempio precedente il numero di gradi di libertà è quattro per ogni polinomio (poichè di grado 3), quindi in tutto otto.</p>
...
<p>La matrice di dimensione $n \times n$ $S_{\lambda}$ è nota  come **smoother matrix**.</p>
<p> **Proprietà:**</p>
* é una matrice $N \times N$ simmetrica di rango $N$
* è semi-positiva definita, $\forall x \neq 0$  $xS_\lambda x \ge 0$
...
<p>La somma degli elementi diagonali della Smoothing Matrix rappresenta il numero dei gradi di libertà. </p>
...
<p> Dal momento  che $df_\lambda = trace(S_{\lambda})$ è monotona in $\lambda$ è possibile inverire la relazione e ricavare $\lambda$.</p>
...
<p>La **varianza** si riferisce alla variazione del valore della funzione stimata $\hat f$ al variare dei dati di training che vengono utilizzati, se un metodo presenta una varianza elevata piccole variazioni nei dati di training possono portare a grandi variazioni in $\hat f$ </p>
...
* $Cov(\hat f) = S_\lambda S_\lambda ^T$
...
<p>Utilizzando per l'ottimizzazione questa penalità si ottiene una superficie bidimensionale smussata, nota come spline a superficie sottile. Condivide molte proprietà con la smoothing spline unidimensionale.</p>
...
n_features = length( names(data))
print(n_features)
par(mfrow=c(3,3))

for (xlabel in names(data)[2:4]) {
  for (ylabel in  names(data)[2:4]) {
    if(xlabel != ylabel) {

      x = data[[xlabel]]
      y = data[[ylabel]]
     # print(x)
      plot(x,y,xlab = '',ylab = '',type = "p",col=alpha('darkblue', 0.2),pch =16,cex=0.8)
    } else{
...
```
<p> Dal grafico  si può notare infatti che come previsto che con un numero di gradi di libertà di 2 il parametro labmda assume un elevato valore smussando, la curva risultante sarà quindi una retta( curva blu), al contrario  con un numero di gradi di libertà pari al numero di osservazioni il parametro $\lambda$ avrà un valore vicino a  0 e la curva risultante sarà molto irregolare (curva rossa).</p>
...
Richiamando la funzione smooth.spline non specificando i parametri di smoothing la funzione stimerà tali  parametri  automaticamente mediante  una Leave-One Out o una Cross Validation Generalizzata, Con $cv$ = TRUE verrà utilizza  una cross-validation Leave-One-Out</p>
...
lines(spline,lwd=2,col="purple")
legend("bottomright",paste("df=",spline$df ,'lambda',"=",format(spline$lambda,scientific = TRUE)),col="purple",lwd=2)
